...
import { Check, Copy, ExternalLink, FileText, FolderUp, Trash2, Upload } from 'lucide-react';
...
export function ClientPack({ customer, onChanged }) {
  ...
  async function copyLink() {
    try {
      ...
      setLink(url);
      await copyText(url);
      toast('Client link copied — send it to staff or the printer');
    } catch (error) {
  ...
  async function remove(id) {
    if (!window.confirm('Delete this file from the dashboard? The network file stays on the store computer.')) return;
    try {
      await deleteArtifact(id);
      toast('Removed from dashboard');
      onChanged?.();
    } catch (error) {
      toast(error.response?.data?.message || 'Could not delete file');
    }
  }
        ...
        files.map((file) => (
          <div className={`pack-row ${file.proof_status || ''}`} key={file.id}>
            ...
            <div className="pack-ops">
              ...
              <button type="button" className="btn-danger" onClick={() => remove(file.id)}>
                <Trash2 />
              </button>
            </div>
          </div>
