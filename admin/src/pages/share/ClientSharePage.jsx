...
export default function ClientSharePage() {
  ...
  if (loading) return <div className="share"><p className="share-msg">Loading client pack…</p></div>;
  if (error || !pack) return <div className="share"><p className="share-msg">{error || 'Not found'}</p></div>;
    ...
    <div className="share">
      <header className="share-head">
        <div>
          ...
          <h1>{customer.name}</h1>
          <p className="sub">{[customer.company, customer.phone, customer.email].filter(Boolean).join(' · ') || 'Print shop client'}</p>
        </div>
      ...
      <section className="share-card">
        ...
        {artifacts.length === 0 ? <p className="muted">No files uploaded yet.</p> : groupByFolder(artifacts).map(([folder, files]) => (
            ...
            <div className="grid">
              {files.map((file) => (
                <article key={file.id} className={`art ${file.proof_status || ''}`}>
                  <div className="thumb">
                    {isImage(file) ? <img src={file.file_url} alt={file.file_name} /> : isPdf(file) ? <iframe title={file.file_name} src={file.file_url} /> : <span>FILE</span>}
                  </div>
                  <div className="info">
                    <b>{file.file_name}</b>
                    ...
                    <a href={file.file_url} target="_blank" rel="noreferrer">Preview</a>
                  </div>
                </article>
              ))}
            </div>
      ...
      <section className="share-card">
        ...
        {jobs.length === 0 ? <p className="muted">No jobs for this client yet.</p> : jobs.map((job) => (
          <article key={job.id} className="job">
            <div className="job-top"><b>{job.job_number}</b><span>{job.stage?.name || job.status}</span></div>
            <p>{job.title}</p>
            <p className="muted">{[job.product_type, job.print_type, job.quantity != null ? `×${job.quantity}` : null, job.size_details].filter(Boolean).join(' · ')}</p>
            {(job.artworks || []).length ? (
