...
export function JobDrawer({
  ...
  useEffect(() => {
    ...
    setFiles([]);
    setProgress({});
    setNetworkPath(job?.customer?.network_folder || prefillCustomer?.network_folder || prefill?.network_folder || '');
    setError('');
    ...
    if (job?.id) {
      ...
      getJob(job.id)
        .then((full) => {
          if (!cancelled && full) {
            setForm(formFromJob(full, { assigned_to: defaultAssignee }));
      ...
      setForm({
        ...
        customer_name: prefill?.customer_name || prefillCustomer?.name || '',
        title: prefill?.title || '',
